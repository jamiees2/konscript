...
module.exports = function parse (line) {
    ...
    var valid = ['kdef', 'enda', 'enda&', 'kall', '.kall', 'punktur', 'konni', 'komment', 'quiet', 'loud', 'rly', 'but', 'many', 'med', 'kon', 'konnict', 'maybe'];
    var validKeys = {'er': ' === ', 'not': ' !== ', 'and':  ' && ', 'or':  ' || ', 'next':  '; ', 'as':  ' = ', 'more':  ' += ', 'less':  ' -= ', 'lots': ' *= ', 'few': ' /= ', 'very': ' var ', 'smaller': ' < ', 'bigger': ' > ', 'smallerish': ' <= ', 'biggerish': ' >= ', 'notrly': ' ! '};
    var statement = '';

    if (keys === null) return line + '\n';

    // not dogescript, such javascript
    if (valid.indexOf(keys[0]) === -1 && keys[1] !== 'er' && keys[1] !== 'punktur' || multiComment && keys[0] !== 'loud') return line + '\n';
